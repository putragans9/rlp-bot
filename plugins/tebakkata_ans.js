...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*hmy/i.test(m.quoted.contentText)) return !0
    ...
    if (m.quoted.id == this.tebakkata[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, `*Benar!* +${this.tebakkata[id][2]} XP`, watermark, 'Tebak Kata', '.tebakkata', m)
