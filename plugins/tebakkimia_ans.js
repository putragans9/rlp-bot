...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*lah/i.test(m.quoted.contentText)) return !0
    ...
    if (m.quoted.id == this.tebakkimia[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.name.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, `*Benar!* +${this.tebakkimia[id][2]} XP`, watermark, 'Tebak Kimia', '.tebakkimia', m)
