...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !/Ketik.*laly/i.test(m.quoted.contentText)) return !0
    ...
    if (m.quoted.id == this.tebakkabupaten[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.title.toLowerCase().trim()) {
            ...
            await this.sendButton(m.chat, `*Benar!* +${this.tebakkabupaten[id][2]} XP`, watermark, 'Tebak Kabupaten', '.tebakkabupaten', m)
